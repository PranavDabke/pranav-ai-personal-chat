# Pranav Dabke — AI Personal (Voice Agent + RAG Chat)

An end-to-end AI Personal that recruiters can **call** and **chat with** to learn about
...
- **Booking:** real Google Calendar via [Google Appointment Scheduling](https://calendar.app.google/6teCw8vXkLaw8G1v8).
...
flowchart TD
    subgraph Voice["Part A — Voice Agent (Vapi)"]
        ...
        V1 --> V2["Claude Haiku 4.5<br/>(Personal system prompt)"]
        V2 --> V3["Vapi voice (TTS)"]
        V2 --> V4["Google Calendar booking"]
    end
    ...
    subgraph Chat["Part B — RAG Chat (Streamlit + Gemini)"]
        ...
        R2 --> R3["gemini-2.5-flash<br/>(+ flash-lite fallback)"]
        R3 --> U1
    end
...
| Answer generation (`gemini-2.5-flash`) | per message | ~2,000 in / ~200 out | ~$0.0003 |
...
- **Voice:** Vapi (Deepgram STT, Claude Haiku 4.5, ElevenLabs TTS) + Google Calendar
- **Chat:** Streamlit, Google Gemini (`gemini-2.5-flash` with `gemini-2.5-flash-lite` fallback, `gemini-embedding-001`), NumPy cosine retrieval
- **Calendar:** Google Calendar + Appointment Scheduling, auto Google Meet (Mon–Fri, 9am–5pm IST)
